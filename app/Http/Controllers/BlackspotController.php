...
class BlackspotController extends Controller
{
    ...
    public function generateBlackspotData()
    {
...
//                $dbblackspot -> jenis_langgar_pertama_id='0';
    ...
    public function dataBlackspot2($input = [])
    {
    ...
    public function dataReport(Request $request)
    {
        ...
        $blackspot = [];
        //DB::enableQueryLog();
        $blackspot = self::dataBlackspot2($input);
        //dd(DB::getQueryLog());
        ...
        return view('site.report2', compact('test','map', 'negeri', 'daerah', 'nolaluan'),[
            'filter' => $filter,
            'test'=>$test,
            'pagination'=> $paginate,
            'tahun' => $getyear,
    ...
    // EXCEL FILE DOWNLOAD
    public function blackexportexcel(Request $request)
    {
        if (!empty($request->all())) {
            $input = $request->all();
        } else {
        ...
        return view('site/blackexcelaccident', compact('blackspot'));
    }

    // CSV FILE DOWNLOAD
    public function blackexportcsv(Request $request)
    {
        if (!empty($request->all())) {
            $input = $request->all();
        } else {
        ...
        $blackspot = self::dataBlackspot2($input);
        return view('site/blackcsvaccident', compact('blackspot'));
    }

    // JSON FILE DOWNLOAD
    public function blackexportjson(Request $request)
    {
        if (!empty($request->all())) {
            $input = $request->all();
        } else {
        ...
        return view('site/blackjsonaccident',[
            'blackspot' => $blackspot
        ]);
    }

    // PDF FILE DOWNLOAD
    public function blackexportpdf(Request $request)
    {
        if (!empty($request->all())) {
            $input = $request->all();
        } else {
        ...
        $pdf = \PDF::loadView('site/blackpdfaccident', compact('blackspot'));
    ...
    public function dataBlackspot(Request $request)
    {
        ...
        return view('site.report3_new', compact('map', 'blackspot', 'negeri', 'daerah', 'nolaluan'), [
            'filter' => $filter,
            'zoomnegeri'=>$zoomnegeri,
            'keluasan' => $keluasan,
            'list_tahun' => $list_tahun,
