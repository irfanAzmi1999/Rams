...
class StlController extends Controller
{
    ...
    public function generateStlData()
    {
...
//                $dbblackspot -> jenis_langgar_pertama_id='0';
    ...
    public function dataStl2($input = [])
    {
    ...
    public function dataReport(Request $request)
    {
        ...
        return view('site.reportstl2', compact('test','map', 'negeri', 'daerah', 'nolaluan'),[
            'filter' => $filter,
            'test'=>$test,
            'pagination'=> $paginate,
            'tahun' => $getyear,
    ...
    // EXCEL FILE DOWNLOAD
    public function stlexportexcel(Request $request)
    {
        if (!empty($request->all())) {
            $input = $request->all();
        } else {
        ...
        return view('site/stlexcelaccident', compact('blackspot'));
    }

    // CSV FILE DOWNLOAD
    public function stlexportcsv(Request $request)
    {
        if (!empty($request->all())) {
            $input = $request->all();
        } else {
        ...
        $blackspot = self::dataStl2($input);
        return view('site/stlcsvaccident', compact('blackspot'));
    }

    // JSON FILE DOWNLOAD
    public function stlexportjson(Request $request)
    {
        if (!empty($request->all())) {
            $input = $request->all();
        } else {
        ...
        return view('site/stljsonaccident',[
            'blackspot' => $blackspot
        ]);
    }

    // PDF FILE DOWNLOAD
    public function stlexportpdf(Request $request)
    {
        if (!empty($request->all())) {
            $input = $request->all();
        } else {
        ...
        $pdf = \PDF::loadView('site/stlpdfaccident', compact('blackspot'));
    ...
    public function dataStl(Request $request)
    {
        ...
        return view('site.reportstl3_new', compact('map', 'blackspot', 'negeri', 'daerah', 'nolaluan'), [
            'filter' => $filter,
            'zoomnegeri'=>$zoomnegeri,
            'keluasan' => $keluasan,
            'list_tahun' => $list_tahun,
	...
	public function dataMapStl(Request $request)
    {
        if (!empty($request->all())) {
            $input = $request->all();
        } else {
